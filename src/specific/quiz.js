import { qpull, qsearch } from "../main.js";
...
const load = async () => {
    ...
    for(let i = 0; i<arr.length; i++){
        ...
        clone.querySelector('a').href = `/quizview.html?id=${arr[i].id}`;
...
const displaySearch = async () => {
    ...
    let response = await qsearch([sTitle, sTopic]);
    clear();
    for(let i = 0; i<response.length; i++){
        ...
        clone.querySelector('a').href = `/quizview.html?id=${response[i].id}`;
        holder.appendChild(clone);
    }
}
